...
DEFAULT_OUT_DIR = WORKSPACE_ROOT / "domain_split"
...
def usable_for_week(row: Dict[str, str], week: int) -> bool:
    for idx in range(1, 11):
        if not math.isfinite(to_float(row.get(f"f{idx}_w{week}", ""))):
            return False
    return math.isfinite(to_float(row.get(f"lifetime_weeks_EOL70", "")))
...
def pick_ft_cells_spread(
    target_groups: Sequence[Dict[str, object]],
...
def build_group_rows(rows: Sequence[Dict[str, str]], y_col: str) -> List[Dict[str, object]]:
    ...
    for group_num, items in grouped.items():
        lifetimes = [to_float(r[y_col]) for r in items if math.isfinite(to_float(r[y_col]))]
        if not lifetimes:
            continue
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Build deterministic target-spread protocol splits for week6-10.")
    ...
    ap.add_argument("--weeks", type=str, default="6,7,8,9,10")
    ap.add_argument("--y_col", type=str, default="lifetime_weeks_EOL70")
    ap.add_argument("--train_group_frac", type=float, default=0.70)
    ap.add_argument("--ft_cell_frac", type=float, default=0.15)
    return ap


def main() -> None:
    args = build_arg_parser().parse_args()
    rows = read_rows(Path(args.data_csv))
    ...
    for week in weeks:
        ...
        if not usable_rows:
            raise ValueError(f"No usable cells for week{week}.")

        group_rows = build_group_rows(usable_rows, str(args.y_col))
        train_end = round_half_up(len(group_rows) * float(args.train_group_frac))
        ...
        ft_quota = round_half_up(len(usable_rows) * float(args.ft_cell_frac))
        ...
        split_rows.sort(
            key=lambda r: (
                str(r["split"]),
                int(to_float(r["group_num"])),
                int(to_float(r["cell_idx"])),
                str(r["cell"]),
            )
        )
        ...
        split_summary = [
            {
                ...
                "train_group_frac": float(args.train_group_frac),
                "ft_cell_frac": float(args.ft_cell_frac),
            }
        ]

        split_csv = out_dir / f"cell_split_targetspread_w{week}_EOL70.csv"
        group_csv = out_dir / f"group_split_targetspread_w{week}_EOL70.csv"
        summary_csv = out_dir / f"split_summary_targetspread_w{week}_EOL70.csv"
        ...
        manifest_rows.append(
            {
                ...
                "usable_cells": len(usable_rows),
                "train_cells": split_summary[0]["train_cells"],
                "fine_tune_cells": split_summary[0]["fine_tune_cells"],
                "test_cells": split_summary[0]["test_cells"],
            }
        )

    write_rows(out_dir / "weekly_protocol_split_manifest_EOL70.csv", manifest_rows, list(manifest_rows[0].keys()))
