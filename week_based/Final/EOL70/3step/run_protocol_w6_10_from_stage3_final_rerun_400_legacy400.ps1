param(
  ...
  [switch]$Cpu,
  [string]$PythonExe = '',
  [string]$OutRoot = 'E:\Datasets\IVAS\Codes\chunqiu_codes\week_based\Final\EOL70\3step\outputs\protocol_w6_10_from_stage3_final_rerun_400_legacy400'
)
...
foreach ($path in @($py, $data, $group, $week5Reference)) {
...
foreach ($week in $Weeks) {
  $split = Join-Path $splitRoot "cell_split_targetspread_w${week}_EOL70.csv"
...
if ($missingWeeks.Count -gt 0) {
  $weekArg = (($missingWeeks | ForEach-Object { [string]$_ }) -join ',')
  Invoke-PythonChecked "$root3\build_protocol_w6_10_EOL70_splits.py" `
    --data_csv $data `
    --out_dir $splitRoot `
    --weeks $weekArg `
...
@(
  'Protocol-based week6-10 comparison from stage3_final_rerun_400 legacy400 protocol.',
  "Week5 reference: $week5Reference",
  "Data CSV: $data",
  "Split root: $splitRoot",
  'Design: use the same deterministic target-spread split protocol as week5 final_rerun_400 on each week-specific usable-cell set.',
  'This fixes the split rule/proportions, not the actual cell identities.',
...
Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Protocol split root: $splitRoot"

foreach ($week in $Weeks) {
  if ($week -lt 6 -or $week -gt 10) {
    throw "This protocol comparison script is intended for weeks 6-10. Got: $week"
  }
  ...
  if (-not (Test-Path -LiteralPath $split)) {
    throw "Missing protocol split CSV for week$week`: $split"
  }
  if ($split -match 'High_tail|random') {
    throw "Refusing to run protocol comparison with non-protocol split: $split"
  }

  $outWeek = Join-Path $OutRoot ("week{0}" -f $week)
  New-Item -ItemType Directory -Force -Path $outWeek | Out-Null
  Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] ===== week$week protocol legacy400 Stage1/2/3 ====="
  Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Candidate CSV: $candidate"
  Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Split CSV: $split"
  ...
  if (-not (Test-Path -LiteralPath (Join-Path $outWeek 'stage1\stage1_top_source_checkpoints.csv'))) {
    Invoke-PythonChecked "$root3\stage1_source_search_optuna_legacy.py" `
      ...
      --study_name "stage1_w${week}_protocol_from_rerun400_legacy400" `
      --n_trials 80 `
      --top_k 5 `
  ...
  if (-not (Test-Path -LiteralPath (Join-Path $outWeek 'stage2\stage2_best_configs.csv'))) {
    Invoke-PythonChecked "$root3\stage2_finetune_search_optuna_legacy.py" `
      ...
      --study_name "stage2_w${week}_protocol_from_rerun400_legacy400" `
      --n_trials 80 `
      --top_k 5 `
      --y_col "lifetime_weeks_EOL70" `
      --dropout 0.0 `
...
Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] All requested protocol weeks finished."
