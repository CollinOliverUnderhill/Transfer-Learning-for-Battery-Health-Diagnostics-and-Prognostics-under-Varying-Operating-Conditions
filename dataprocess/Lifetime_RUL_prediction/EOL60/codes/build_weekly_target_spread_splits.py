...
SOURCE_CSV = Path(r"E:\Datasets\IVAS\Processing_Data_dd_exclude\EOL60\cell_split_by_lifetime_EOL60_augmented_weeks.csv")
WEEKS = (3, 5, 10, 15)
TARGET_COL = "lifetime_weeks_EOL60"
        ...
        group_rows.append(
            {
                ...
                "lifetime_median": sorted(lifetimes)[len(lifetimes) // 2] if len(lifetimes) % 2 == 1 else (sorted(lifetimes)[len(lifetimes)//2 - 1] + sorted(lifetimes)[len(lifetimes)//2]) / 2.0,
...
def main() -> None:
    ...
    write_rows(
        features_dir / "feature_table_all_cells_multiweek_EOL60.csv",
        rows,
        list(rows[0].keys()),
    )
    ...
    for week in WEEKS:
        ...
        split_csv = domain_dir / f"cell_split_targetspread_w{week}_EOL60.csv"
        group_csv = domain_dir / f"group_split_targetspread_w{week}_EOL60.csv"
        summary_csv = domain_dir / f"split_summary_targetspread_w{week}_EOL60.csv"
    ...
    write_rows(domain_dir / "weekly_split_manifest_EOL60.csv", manifest_rows, list(manifest_rows[0].keys()))
